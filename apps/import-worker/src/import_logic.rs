...
pub async fn import_file(
    ...
    let raw = tokio::fs::read_to_string(input_path).await.map_err(|_| {
        AppError::validation(format!("input file not found: {input_path}"), request_id)
    })?;

    let file: ImportFile = serde_json::from_str(&raw).map_err(|_| {
        AppError::validation("input file is not valid JSON".to_string(), request_id)
    })?;
    ...
    for (i, product) in file.products.iter().enumerate() {
        ...
        match catalog
            .create_product(ProductCreate::from(product), ids, clock)
            .await
        {
            Ok(_) => created += 1,
