...
async fn set_then_get_returns_the_stored_value() {
    ...
    cache.set_json(&key, &Sample { value: "hello".into() }, Duration::from_secs(30)).await;

    let got: Option<Sample> = cache.get_json(&key).await;
    assert_eq!(got, Some(Sample { value: "hello".into() }));
...
async fn delete_removes_the_key() {
    ...
    cache.set_json(&key, &Sample { value: "to-remove".into() }, Duration::from_secs(30)).await;
    assert_eq!(cache.get_json::<Sample>(&key).await, Some(Sample { value: "to-remove".into() }));
...
async fn set_respects_ttl_expiry() {
    ...
    cache.set_json(&key, &Sample { value: "short-lived".into() }, Duration::from_secs(1)).await;
    assert_eq!(cache.get_json::<Sample>(&key).await, Some(Sample { value: "short-lived".into() }));
...
async fn get_returns_none_when_redis_is_unreachable_rather_than_erroring() {
    ...
    assert_eq!(got, None, "an unreachable Redis must look like a cache miss, not a crash");
}

#[tokio::test]
async fn set_does_not_panic_or_block_forever_when_redis_is_unreachable() {
    let cache = Cache::new(UNREACHABLE_REDIS_URL).expect("valid redis url");
    cache.set_json("anything", &Sample { value: "x".into() }, Duration::from_secs(30)).await;
}
